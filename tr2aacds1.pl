...
use constant VERSION => '2014.05.15'; #'2013.07.27'; # '04.15'; #04.07; 03.14'; # '.03.11'; '.03.06';
...
# use Cwd qw(getcwd cwd abs_path); # use FindBin::cwd2()
...
#  }
  ...
  unless($ENV{TMPDIR}) { ## FIXME sort TMPDIR, cant use /tmp
   ## 2015 perl fails to find FindBin::cwd2() .. revert to use Cwd::getcwd or cwd 
  ...
  while(<F>) { # cd-hit clstr format
    ...
    if(/^>/) { # new cluster, decide if last mainid is good, or replace by 2nd best ?
      ...
      if($mainid and @fragids) {
        my $aqmain= $aq{$mainid}; my($awm,$aw1m,$apm,$aqm)=split",",$aqmain;
        my $okmain= ($aqmain=~/utrbad|utrpoor|partial/ or $mainid =~ /utrorf/)?0:1;
        ## $better{$mainid}=$mainid; # do below when 1st seen
        unless($okmain) { 
          my $awmMIN= 0.90 * $awm; # CONFIG
          @fragids= sort{ $aq{$b} <=> $aq{$a} or $a cmp $b } @fragids;
          ## 2nd longest/aagood replaces 1st longest/aapoor
          for my $fid (@fragids) {
            my $aqfrag= $aq{$fid}; my($awf,$aw1f,$apf,$aqf)=split",",$aqfrag;
            my $okfrag= ($aqmain=~/utrbad|utrpoor|partial/ or $fid =~ /utrorf/)?0:1;
            my $pdif= $apf - $apm; 
            if($okfrag and $awf >= $awmMIN and $pdif >= fPCDS_DIFF) { 
              delete $better{$mainid}; $better{$fid}= $mainid;
              #wrong# $better{$mainid}= $fid; ## join(" ",$fid,"plo:".$mainid); # plo = poor-longer-orf
              $nbetter++; last;
              }
            } 
          }
