...
MAIN: {
  my($hexfile)= shift @ARGV;  ## figure out how to embed this, 4096 rows, 2+ cols, 
    # as  hash{codonhex}=loglike_coding_nocode    .. see below 800 line %hexcodencode data.
  my($cdsin)= shift @ARGV; # foreach cdsin ..
  
  if($doCompareCpat) {
    my $cpattab = $hexfile;
    my $cqualtab= $cdsin;
    my($outfile)= shift @ARGV; $outfile||="";
    die "usage: env cpat=1 $0 cpat.out cds.qual [stdout|outfile] " unless(-f $cpattab and -f $cqualtab);
    ...
    my $usehexinline=0;
    if(-f $hexfile and not -f $cdsin) {
      $cdsin=$hexfile; $hexcode=\%hexcodencode; $usehexinline=1;
    } else {
      die "usage: $0 hexamers input.cds " unless(-f $hexfile and -f $cdsin);
    }
...
sub aaqualscore
{
  ...
  if($mqual =~ /utrbad/) { 
    $mqv = ($mqv >= kAAQUAL_MAX-1 and $aaw>= kAABIG) ? kAAQUAL_NONE
      : $mqv - 2; # change, short aa + utrbad, ignore complete..
    } 
  elsif($mqual =~ /utrpoor/) { $mqv -= 1;  $mqv-- if($aaw<kAATINY); } # $mqv-- if($aaw<kAABIG);
...
sub codingpotClass {
  ...
  if($fclass eq "Noncode" and $aclass ne $fclass and $fclass eq $hclass) {
    if($mqv < kAAQUAL_MAX-1 or $isutrorf or $pcds < 70 ) { $aclass=$fclass; }
    ##  elsif($pcds < 70) { $aclass=$fclass; } # utrpoor and abit above..
    # if mvq == max but other things...
  }
    #? add agreement level? 3,2,1?
  my $tclass= 
    (($aclass eq $fclass) and ($fclass eq $hclass))?$aclass:
    (($aclass eq $fclass) or ($aclass eq $hclass))?$aclass:
...
sub makeCdsQual {
  ...
  sub puta { 
    ...
    if($al =~ /na/ or not $al) 
    {
      ...
      if($pcds <= $pCDSpoor) { $compl.= ($pcds <= $pCDSbad)?"-utrbad":"-utrpoor"; }
      $al="$aaw,$pcds%,$compl";
    }
