...
use constant VERSION => '2016.05.03'; ## blast ncpu fail retry
...
our $okayclass  ='main|noclass|dropmain';
...
my ($debug,$tidyup,$runsteps,$USE_LASTZ)= (0) x 9; # pm: $dryrun, 
...
#  }
  ...
  ($aasize)= make_aaqual($aaseq); # save global hash in $AAQUALH= make_aaqual($aasize) ??
...
#   my $cmd="grep '^>' $cdsnrseq > $cdsnrhdr"; # dont really need .hdr file; combine below
  ...
  our $aaqualh= make_aaqual($aaqual); # ret: ($qualh,$sizeh,$naa) : $qualh # may be global hash, read once?
...
sub nofrag_reassignbest
{
  ...
  my $aaqualh= make_aaqual($aaqual); # ret: ($qualh,$sizeh,$naa) : $qualh # may be global hash, read once?
...
sub make_aaqual
{
  ...
  # my $aasize= makeAaQual($aaseq,1); # use constant makeAaQual_SETHASH=>1;   #this doesn't work because makeAaQual is not a defined function!!
...
sub blastn_cds_ncpu
{
  ...
  if(FIXME_blastn_cds_ncpu_fails) {  
    ...
    my($runerr,$nok,$failset)= blastn_collate_result($npart, $cdsbltab, \@bloset,\@splset);
    if($runerr and ref $failset) {
      ...
      if($nfail >= $npart/2) { 
        ...
        # reuse @bloset or merge @rerunset ?
        my($runerr2,$nok2,$failset2)= blastn_collate_result($npart, $cdsbltab, \@bloset,\@splset);
